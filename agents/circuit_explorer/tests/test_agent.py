...
from agents.circuit_explorer.agent import BooleanFunction, CircuitExplorerAgent, CircuitModel
from core.complexity_models.circuit import CircuitClass


def _fn(name: str, n: int, evaluator):
...
def test_explore_and_function_finds_small_circuit(tmp_path: Path) -> None:
    ...
    report = agent.explore(fn, [CircuitModel(CircuitClass.MONOTONE, max_size=4, max_depth=2)])
...
def test_parity_monotone_known_impossibility() -> None:
    ...
    rec = agent.explore(fn, [CircuitModel(CircuitClass.MONOTONE, max_size=5, max_depth=3)]).records[0]
...
def test_acc0_mod_finds_parity_small() -> None:
    ...
    rec = agent.explore(fn, [CircuitModel(CircuitClass.ACC0, max_size=5, max_depth=3, moduli=(2,))]).records[0]
...
def test_serialization_roundtrip(tmp_path: Path) -> None:
    ...
    report = agent.explore(fn, [CircuitModel(CircuitClass.AC0, max_size=4, max_depth=2)])
...
def test_structural_invariants_for_xor() -> None:
    ...
    rec = agent.explore(fn, [CircuitModel(CircuitClass.AC0, max_size=8, max_depth=4)]).records[0]
    assert rec.structural_invariants["symmetry_group_size"] == 2
...
def test_structural_invariants_for_and_or_majority() -> None:
    ...
    recs += agent.explore(maj_fn, [CircuitModel(CircuitClass.ACC0, 7, 3, moduli=(2, 3))]).records
