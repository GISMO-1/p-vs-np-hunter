...
class CircuitExplorerAgent:
    ...
    def explore(self, function: BooleanFunction, models: list[CircuitModel]) -> CircuitReport:
        records: list[CircuitRecord] = []
        target_table = function.truth_table() if function.arity() <= self.config["truth_table_limit"] else None
        ...
        report = CircuitReport(
            ...
            truth_table={"".join("1" if b else "0" for b in k): v for k, v in target_table.items()} if target_table else None,
            records=records,
            output_path="",
        )
    ...
    def _explore_model(self, function: BooleanFunction, model: CircuitModel, target_vector: tuple[bool, ...]) -> CircuitRecord:
    ...
    def _target_vector(self, function: BooleanFunction) -> tuple[bool, ...]:
        return tuple(function.evaluator(dict(zip(function.variable_names, bits, strict=True))) for bits in product((False, True), repeat=function.arity()))

    def _enumerate_exhaustive(self, function: BooleanFunction, model: CircuitModel, target: tuple[bool, ...]) -> tuple[_Node | None, dict[int, int]]:
        ...
        for current_size in range(2, model.max_size + 1):
            additions = self._expand_nodes(nodes, by_table, function, model, current_size)
            ...
            for node in additions:
                if node.table == target and node.depth <= model.max_depth:
                    ...
                    if found is None or node.size < found.size or (node.size == found.size and node.depth < found.depth):
                        found = node
        return found, curve

    def _enumerate_pruned(self, function: BooleanFunction, model: CircuitModel, target: tuple[bool, ...]) -> tuple[_Node | None, dict[int, int]]:
        nodes, by_table = self._base_nodes(function)
        ...
        for size in range(2, model.max_size + 1):
            ...
            additions = sorted(additions, key=lambda n: (len(n.deps), n.depth, n.expr))[:rng_budget]
            nodes.extend(additions)
            for node in additions:
    ...
    def _base_nodes(self, function: BooleanFunction) -> tuple[list[_Node], dict[tuple[bool, ...], _Node]]:
        n = function.arity()
        nodes: list[_Node] = []
        ...
        for idx, name in enumerate(function.variable_names):
            ...
            node = _Node(table=vec, size=1, depth=0, deps=frozenset({idx}), expr=name, kind="INPUT")
            nodes.append(node)
            by_table.setdefault(vec, node)
        for value in (False, True):
            vec = tuple(value for _ in range(2**n))
            node = _Node(table=vec, size=1, depth=0, deps=frozenset(), expr="1" if value else "0", kind="CONST")
            nodes.append(node)
            by_table.setdefault(vec, node)
        return nodes, by_table

    def _expand_nodes(self, nodes: list[_Node], by_table: dict[tuple[bool, ...], _Node], function: BooleanFunction, model: CircuitModel, current_size: int) -> list[_Node]:
        ...
        for a, b in combinations_with_replacement(nodes, 2):
            ...
            for kind, op in self._binary_ops(model):
                ...
                node = _Node(vec, current_size, max(a.depth, b.depth) + 1, a.deps | b.deps, f"{kind}({a.expr},{b.expr})", kind)
            ...
            if model.cls == CircuitClass.ACC0:
                for mod in model.moduli:
                    for residue in range(mod):
                        vec = tuple(((int(x) + int(y)) % mod) == residue for x, y in zip(a.table, b.table, strict=True))
                        node = _Node(vec, current_size, max(a.depth, b.depth) + 1, a.deps | b.deps, f"MOD{mod}_{residue}({a.expr},{b.expr})", "MOD")
                        if node.depth <= model.max_depth and vec not in by_table:
                            by_table[vec] = node
                            additions.append(node)
        if model.cls in {CircuitClass.AC0, CircuitClass.ACC0}:
            for source in nodes:
                ...
                node = _Node(vec, current_size, source.depth + 1, source.deps, f"NOT({source.expr})", "NOT")
                if node.depth <= model.max_depth and vec not in by_table:
                    by_table[vec] = node
                    additions.append(node)
        return additions

    def _binary_ops(self, model: CircuitModel) -> Iterable[tuple[str, Callable[[bool, bool], bool]]]:
        ops: list[tuple[str, Callable[[bool, bool], bool]]] = [("AND", lambda x, y: x and y), ("OR", lambda x, y: x or y)]
        if model.cls == CircuitClass.MONOTONE:
            return ops
        return ops

    def _compute_invariants(self, function: BooleanFunction, witness: _Node | None, curve: dict[int, int]) -> dict[str, Any]:
    ...
    def _symmetry_group_size(self, function: BooleanFunction, table: Mapping[BitVector, bool]) -> int:
        n = function.arity()
    ...
    def _gate_elimination(self, witness: _Node | None, function: BooleanFunction) -> list[dict[str, Any]]:
        if witness is None:
            return []
        ...
        if keep_ratio >= threshold:
            return [{"gate": witness.expr, "unchanged_ratio": keep_ratio, "rationale": "Expression support suggests redundancy under random restrictions."}]
        return []
    ...
    def _switching_cross_check(self, function: BooleanFunction, witness: _Node | None) -> dict[str, float] | None:
        if witness is None or function.arity() > self.config["switching_check_variable_limit"]:
            return None
        ...
        return {"p": p, "t": float(t), "tail_probability": exact_tail, "upper_bound": upper}
    ...
    def _truth_table_to_dnf(self, function: BooleanFunction) -> DNF:
        ...
        for bits, value in function.truth_table().items():
            if not value:
                continue
            term = tuple((var, bit) for var, bit in zip(function.variable_names, bits, strict=True))
            terms.append(term)
        return DNF(terms=tuple(terms))

    def _write_report(self, report: CircuitReport) -> Path:
        path = Path(self.config["output_dir"]) / f"{report.function_name}_{report.timestamp.replace(':', '-')}.json"
